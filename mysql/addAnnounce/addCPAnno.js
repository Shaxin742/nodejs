...
fs.readFile("./cp1.json", function (err, data) {
  if (err) { console.log(err) }
  else {
    console.log(data)
    data = JSON.parse(data)
    // data.map(v => {
    //   for (var i =0; i < v.endTime; i++) {
    ...
    for (var n = 0; n < data.length; n += 2) {
      let beginTime = transTimeToYM(data[n].start_time)
      let endTime = transTimeToYM(data[n + 1].start_time)
      for (var i = beginTime; i < endTime; i++) {
        ...
        console.log(Cur)
        console.log(transToTimestamp(Cur))
        let insertData = [transToTimestamp(Cur), 1, '结算报表', '本月结算报表已生成', data[n].org_id, data[n].org_name, 0]
        insertTable(insertData)
      }
    }
  }
